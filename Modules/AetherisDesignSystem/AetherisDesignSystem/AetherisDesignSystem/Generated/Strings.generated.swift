// swiftlint:disable all
// Generated using SwiftGen — https://github.com/SwiftGen/SwiftGen

import Foundation

// swiftlint:disable superfluous_disable_command file_length implicit_return prefer_self_in_static_references

// MARK: - Strings

// swiftlint:disable explicit_type_interface function_parameter_count identifier_name line_length
// swiftlint:disable nesting type_body_length type_name vertical_whitespace_opening_braces
internal enum Strings {
  internal enum CardDistribution {
    /// Enjoy exclusive benefits with zero annual fee.
    internal static let creditCardSubtitle = Strings.tr("Localizable", "CardDistribution.creditCardSubtitle", fallback: "Enjoy exclusive benefits with zero annual fee.")
    /// Credit Card
    internal static let creditCardTitle = Strings.tr("Localizable", "CardDistribution.creditCardTitle", fallback: "Credit Card")
    /// Grow your money with flexible plans.
    internal static let investmentsSubtitle = Strings.tr("Localizable", "CardDistribution.investmentsSubtitle", fallback: "Grow your money with flexible plans.")
    /// Investments
    internal static let investmentsTitle = Strings.tr("Localizable", "CardDistribution.investmentsTitle", fallback: "Investments")
    /// Personalized loan options.
    internal static let loanSubtitle = Strings.tr("Localizable", "CardDistribution.loanSubtitle", fallback: "Personalized loan options.")
    /// Loan
    internal static let loanTitle = Strings.tr("Localizable", "CardDistribution.loanTitle", fallback: "Loan")
  }
  internal enum CardInfo {
    /// See Insights
    internal static let monthlySpendingButton = Strings.tr("Localizable", "CardInfo.monthlySpendingButton", fallback: "See Insights")
    /// Top category: Restaurants 🍔
    internal static let monthlySpendingCaption = Strings.tr("Localizable", "CardInfo.monthlySpendingCaption", fallback: "Top category: Restaurants 🍔")
    /// Monthly Spending
    internal static let monthlySpendingHeadline = Strings.tr("Localizable", "CardInfo.monthlySpendingHeadline", fallback: "Monthly Spending")
    /// You spent $2,310 in August
    internal static let monthlySpendingTitle = Strings.tr("Localizable", "CardInfo.monthlySpendingTitle", fallback: "You spent $2,310 in August")
  }
  internal enum CardView {
  }
  internal enum CardsMock {
    /// Monthly Spending
    internal static let monthlySpendingHeadline = Strings.tr("Localizable", "CardsMock.monthlySpendingHeadline", fallback: "Monthly Spending")
    /// You spent $2,310 in August
    internal static let monthlySpendingTitle = Strings.tr("Localizable", "CardsMock.monthlySpendingTitle", fallback: "You spent $2,310 in August")
  }
  internal enum CreditCard {
    /// MASTERCARD
    internal static let brandMastercard = Strings.tr("Localizable", "CreditCard.brandMastercard", fallback: "MASTERCARD")
    /// VISA
    internal static let brandVisa = Strings.tr("Localizable", "CreditCard.brandVisa", fallback: "VISA")
    /// Jorge Henrique
    internal static let ownerName = Strings.tr("Localizable", "CreditCard.ownerName", fallback: "Jorge Henrique")
    /// Amado batista
    internal static let ownerNameTwo = Strings.tr("Localizable", "CreditCard.ownerNameTwo", fallback: "Amado batista")
    /// Valid till
    internal static let validTill = Strings.tr("Localizable", "CreditCard.validTill", fallback: "Valid till")
  }
  internal enum FullScreenError {
    /// We couldn't load your information. Please check your connection and try again.
    internal static let description = Strings.tr("Localizable", "FullScreenError.description", fallback: "We couldn't load your information. Please check your connection and try again.")
    /// Try again
    internal static let primaryButton = Strings.tr("Localizable", "FullScreenError.primaryButton", fallback: "Try again")
    /// Try later
    internal static let secondaryButton = Strings.tr("Localizable", "FullScreenError.secondaryButton", fallback: "Try later")
    /// Something went wrong
    internal static let title = Strings.tr("Localizable", "FullScreenError.title", fallback: "Something went wrong")
  }
  internal enum GlowButton {
    /// Continue
    internal static let `continue` = Strings.tr("Localizable", "GlowButton.continue", fallback: "Continue")
    /// Continue
    internal static let continueTitle = Strings.tr("Localizable", "GlowButton.continueTitle", fallback: "Continue")
  }
  internal enum MinimalDropdown {
    /// All Time
    internal static let allTime = Strings.tr("Localizable", "MinimalDropdown.allTime", fallback: "All Time")
    /// This Month
    internal static let thisMonth = Strings.tr("Localizable", "MinimalDropdown.thisMonth", fallback: "This Month")
    /// This Week
    internal static let thisWeek = Strings.tr("Localizable", "MinimalDropdown.thisWeek", fallback: "This Week")
    /// Today
    internal static let today = Strings.tr("Localizable", "MinimalDropdown.today", fallback: "Today")
    /// Transactions History
    internal static let transactionsHistory = Strings.tr("Localizable", "MinimalDropdown.transactionsHistory", fallback: "Transactions History")
  }
  internal enum Profile {
    /// contact@melissamccarthy.com
    internal static let email = Strings.tr("Localizable", "Profile.email", fallback: "contact@melissamccarthy.com")
    /// Feedback
    internal static let feedback = Strings.tr("Localizable", "Profile.feedback", fallback: "Feedback")
    /// General
    internal static let generalSection = Strings.tr("Localizable", "Profile.generalSection", fallback: "General")
    /// Joined August 17, 2025
    internal static let joinedDate = Strings.tr("Localizable", "Profile.joinedDate", fallback: "Joined August 17, 2025")
    /// Notifications
    internal static let notificationsSection = Strings.tr("Localizable", "Profile.notificationsSection", fallback: "Notifications")
    /// (33) 9908-3213
    internal static let phone = Strings.tr("Localizable", "Profile.phone", fallback: "(33) 9908-3213")
    /// Push notifications
    internal static let pushNotifications = Strings.tr("Localizable", "Profile.pushNotifications", fallback: "Push notifications")
    /// SMS notifications
    internal static let smsNotifications = Strings.tr("Localizable", "Profile.smsNotifications", fallback: "SMS notifications")
    /// Melissa Mccarthy
    internal static let userName = Strings.tr("Localizable", "Profile.userName", fallback: "Melissa Mccarthy")
  }
  internal enum TabBar {
    /// Cards
    internal static let cards = Strings.tr("Localizable", "TabBar.cards", fallback: "Cards")
    /// Home
    internal static let home = Strings.tr("Localizable", "TabBar.home", fallback: "Home")
    /// Profile
    internal static let profile = Strings.tr("Localizable", "TabBar.profile", fallback: "Profile")
  }
  internal enum UserView {
    /// Joined August 17, 2025
    internal static let joinedDate = Strings.tr("Localizable", "UserView.joinedDate", fallback: "Joined August 17, 2025")
    /// Melissa Mccarthy
    internal static let userName = Strings.tr("Localizable", "UserView.userName", fallback: "Melissa Mccarthy")
  }
}
// swiftlint:enable explicit_type_interface function_parameter_count identifier_name line_length
// swiftlint:enable nesting type_body_length type_name vertical_whitespace_opening_braces

// MARK: - Implementation Details

extension Strings {
  private static func tr(_ table: String, _ key: String, _ args: CVarArg..., fallback value: String) -> String {
    let format = BundleToken.bundle.localizedString(forKey: key, value: value, table: table)
    return String(format: format, locale: Locale.current, arguments: args)
  }
}

// swiftlint:disable convenience_type
private final class BundleToken {
  static let bundle: Bundle = {
    #if SWIFT_PACKAGE
    return Bundle.module
    #else
    return Bundle(for: BundleToken.self)
    #endif
  }()
}
// swiftlint:enable convenience_type
